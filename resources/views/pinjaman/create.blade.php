<x-app-layout>
    <x-slot name="header">{{ __('Pengajuan Pinjaman Baru') }}</x-slot>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('admin.pinjaman.store') }}" method="POST" id="pinjamanForm">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="tanggal_pengajuan" id="tanggal_pengajuan" value="{{ old('tanggal_pengajuan') }}" class="form-control @error('tanggal_pengajuan') is-invalid @enderror" required>
                        @error('tanggal_pengajuan') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Nominal') }} (Rp)</label>
                        <input type="text" name="nominal" id="nominal" value="{{ old('nominal') ? number_format((float) old('nominal'), 0, ',', '.') : '' }}" class="form-control rupiah-input @error('nominal') is-invalid @enderror" required>
                        @error('nominal') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('Bunga Pinjaman') }} (%)</label>
                        <input type="text" name="bunga_display" id="bunga_input" class="form-control @error('bunga_persen') is-invalid @enderror" list="bunga_list" placeholder="{{ __('Ketik atau pilih bunga') }}" autocomplete="off" value="{{ old('bunga_display') }}">
                        <datalist id="bunga_list">
                            @foreach($bungaPinjaman as $bp)
                                <option value="{{ $bp->nama }} ({{ $bp->bunga }}%)">
                            @endforeach
                        </datalist>
                        <input type="hidden" name="bunga_pinjaman_id" id="bunga_pinjaman_id" value="{{ old('bunga_pinjaman_id') }}">
                        <input type="hidden" name="bunga_persen" id="bunga_persen" value="{{ old('bunga_persen') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tenor" id="tenor" class="form-select @error('tenor') is-invalid @enderror" required>
                            <option value="">- {{ __('Pilih') }} -</option>
                            <option value="1" {{ old('tenor') == 1 ? 'selected' : '' }}>1 {{ __('Bulan') }}</option>
                            <option value="3" {{ old('tenor') == 3 ? 'selected' : '' }}>3 {{ __('Bulan') }}</option>
                            <option value="6" {{ old('tenor') == 6 ? 'selected' : '' }}>6 {{ __('Bulan') }}</option>
                            <option value="12" {{ old('tenor') == 12 ? 'selected' : '' }}>12 {{ __('Bulan') }}</option>
                            <option value="24" {{ old('tenor') == 24 ? 'selected' : '' }}>24 {{ __('Bulan') }}</option>
                            <option value="36" {{ old('tenor') == 36 ? 'selected' : '' }}>36 {{ __('Bulan') }}</option>
                        </select>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary">{{ __('Ajukan Pinjaman') }}</button>
...
@push('scripts')
<script>
    const bungaMap = {!! Illuminate\Support\Js::from($bungaPinjaman->mapWithKeys(fn($bp) => [
