<x-app-layout>
    <x-slot name="header">Create Akun Keuangan</x-slot>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.akun-keuangan.store') }}" method="POST">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="kategori_type" class="form-select" id="kategori_type">
                            ...
                            <option value="aktiva" {{ old('kategori_type') === 'aktiva' ? 'selected' : '' }}>Aktiva</option>
                            <option value="passiva" {{ old('kategori_type') === 'passiva' ? 'selected' : '' }}>Passiva</option>
                        </select>
                    </div>
                    <div class="col-md-6" id="kategori_aktiva_wrapper" style="display: none;">
                        ...
                        <select name="kategori_aktiva_id" class="form-select">
                            ...
                            @foreach($kategoriAktiva as $ka)
                                <option value="{{ $ka->id }}" {{ old('kategori_aktiva_id') == $ka->id ? 'selected' : '' }}>{{ $ka->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6" id="kategori_passiva_wrapper" style="display: none;">
                        ...
                        <select name="kategori_passiva_id" class="form-select">
                            ...
                            @foreach($kategoriPassiva as $kp)
                                <option value="{{ $kp->id }}" {{ old('kategori_passiva_id') == $kp->id ? 'selected' : '' }}>{{ $kp->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
